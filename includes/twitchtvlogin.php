...
class TwitchTV {
  ...
  var $redirect_url = 'https://restream.chisdealhd.co.uk/twitchlogin.php'; //change this value, should be your TwitchTV Application Rerdirect URL
  var $scope_array = array('user_read','user_subscriptions');
     ...
     * @return array         Array of data that includes the display name, Status, Chat links, game that the stream is playing and the banner
     */
    public function get_userlogo($username) {
        //initiate connection to the twitch.tv servers
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_RETURNTRANSFER => 1,
        ...
        if (!$result) {
            ...
            if (!empty($username)) {
                ...
                if ($return['users'][0]['logo'] == null) {
                    $user_id = "https://img.pngio.com/prohibited-logo-no-symbol-sign-svg-free-png-pngfuel-logo-sign-png-350_350.png";
                } else {
                    $user_id = $return['users'][0]['logo'];
                }
                return $user_id;
            }
        }
